...
import { CustomerCheckoutData } from '../types';
...
interface LinkCryptoModalProps {
  initialEmail?: string;
  onClose: () => void;
...
export const LinkCryptoModal: React.FC<LinkCryptoModalProps> = ({
  initialEmail = '',
  onClose,
  onSuccess,
}) => {
          ...
          <div className="space-y-6">
            ...
            <div className="border-b border-emerald-800/80 pb-4">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">
                BUY DIRECT ACCESS LINK ($200 USDT)
              </h2>
              <p className="text-xs text-emerald-500 mt-1">
                Amount Required: <span className="text-white font-bold">$200.00 USDT</span> | 12-Hour Auto Email Dispatch Included
              </p>
            </div>
            ...
            <form onSubmit={handleConfirmPayment} className="space-y-4 pt-2 border-t border-emerald-900">
              ...
              >
                {isProcessing ? (
                  <>
                    <Cpu className="w-4 h-4 animate-spin text-black" /> CONFIRMING PAYMENT & INITIALIZING 12H LINK TIMER...
                  </>
                ) : (
                  <>
                    CONFIRM $200 PAYMENT & INITIALIZE LINK TIMER <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
            </form>
          ...
          <div className="py-8 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div className="p-4 bg-emerald-950/90 rounded-2xl border border-emerald-500/80 text-emerald-200 text-xs sm:text-sm font-mono max-w-lg space-y-2 shadow-lg">
              ...
              <p className="text-xs text-emerald-400/90">
                Direct access link send to: <strong className="text-white">{checkoutData.email || 'your email'}</strong>
              </p>
            </div>
